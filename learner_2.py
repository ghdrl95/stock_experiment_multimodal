...
'''
from network_2 import updateTargetGraph, experience_buffer, updateTarget,TrendPredictNetwork
import os
...
import json
class PolicyLearner:
    def __init__(self,load_model=True, learning_rate = 0.005, min_trading_unit=0, max_trading_unit=30, delayed_reward_threshold=.01, training = True):
        ...
        self.data_type = [Environment.TYPE_BASIC]
        ...
        self.mainQN = [TrendPredictNetwork(learning_rate=learning_rate, model_type=self.network_type, name='main_%s_%s'%(self.network_type,type),data_type=type) for type in self.data_type]
        if training:
            self.targetQN = [TrendPredictNetwork(learning_rate=learning_rate, model_type=self.network_type, name='target_%s_%s' %(self.network_type,type),data_type=type) for type in self.data_type]
