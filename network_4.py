...
class TrendPredictNetwork:
    ...
    RANGE_SHAPE = { 20 : [ [400, 240, 3], [320,235,3],[320,245,3],[320,250,3],[320,215,3],[320,215,3] ] }
        ...
        with tf.device(device):
            ...
            for i, type in enumerate(self.data_type):
                with tf.variable_scope(name+str(type)):
                    ...
                    #?,778,243,32
                    conv1 = tf.contrib.layers.convolution2d(inputs=self.inImage[i], num_outputs=32, kernel_size=[14, 7],
                                                            biases_initializer=None,
                                                            padding="VALID",trainable=is_train)
                    batch_norm_1 = tf.contrib.layers.batch_norm(conv1, is_training=self.phase, scope='bn_1', center=True, scale=True)
                    #?,388,121,32
                    mp1 = tf.contrib.layers.max_pool2d(batch_norm_1, kernel_size=[5, 4],stride=3)
                    # ?,386,119,32
                    conv2 = tf.contrib.layers.convolution2d(inputs=mp1, num_outputs=32, kernel_size=[7, 7],biases_initializer=None,
                                                            padding="VALID",trainable=is_train)
                    batch_norm_2 = tf.contrib.layers.batch_norm(conv2, is_training=self.phase, scope='bn_2', center=True,
                                                                scale=True)
                    # ?,192,59,32
                    mp2 = tf.contrib.layers.max_pool2d(batch_norm_2, kernel_size=[4, 3])

                    conv3 = tf.contrib.layers.convolution2d(inputs=mp2, num_outputs=16, kernel_size=[3, 3],biases_initializer=None,
                                                            padding="VALID",trainable=is_train)
                    batch_norm_3 = tf.contrib.layers.batch_norm(conv3, is_training=self.phase, scope='bn_3', center=True,
                                                                scale=True)
                    mp3 = tf.contrib.layers.max_pool2d(batch_norm_3, kernel_size=[4, 3])
                    conv4 = tf.contrib.layers.convolution2d(inputs=mp3, num_outputs=16, kernel_size=[3, 3],biases_initializer=None,
                                                            padding="VALID",trainable=is_train)
                    ...
                    conv5 = tf.contrib.layers.convolution2d(inputs=mp4, num_outputs=64, kernel_size=[3, 3],biases_initializer=None,
                                                            padding="VALID",trainable=is_train)
                ...
                '''
                    ...
                    img_result = tf.reshape(tf.transpose(batch_norm_4, perm=[0, 2, 1, 3]),
                                            shape=[-1, batch_norm_4.shape[2],
                                                   batch_norm_4.shape[1] * batch_norm_4.shape[3]])

                    # 하나의 LSTML셀에 몇개의 뉴럴을 생성할 것인지 설정
                    lstms=[]
                    for i in range(self.LSTM_LAYERS):
                        lstm_cell = tf.contrib.rnn.LSTMCell(512, state_is_tuple=True,trainable=is_train,name="LSTM_%d"%i)
                    ...
                    fc_layer_1 = slim.fully_connected(batch_norm, 512, activation_fn=tf.nn.leaky_relu,
                                                      weights_initializer=normalized_columns_initializer())

                    dropout = slim.dropout(fc_layer_1, self.keep_per)

                    streamAC, streamVC = tf.split(dropout, [256, 256], 1)
                    ...
                    A_list.append(streamA)
                    V_list.append(streamV)

            with tf.variable_scope(name):
                #concat_Q = tf.concat([Q for Q in Qout_list], 1)
                concat_A = tf.concat([Q for Q in A_list], 1) #tf.keras.layers.add([Q for Q in A_list])
                concat_V = tf.concat([Q for Q in V_list], 1) #tf.keras.layers.add([Q for Q in V_list])

                #AW = tf.Variable(tf.random_normal([256, Agent.NUM_ACTIONS]))
                #VW = tf.Variable(tf.random_normal([256, 1]))
                self.Advantage =slim.fully_connected(concat_A, Agent.NUM_ACTIONS ,biases_initializer=None,activation_fn=None) #tf.matmul(concat_A, AW)
                self.Value = slim.fully_connected(concat_V, 1 ,biases_initializer=None,activation_fn=None)

                # Q = V + A<-(A - A평균)
                # self.Qout = self.Value + tf.subtract(self.Advantage, tf.reduce_mean(self.Advantage, reduction_indices=1, keep_dims=True))
                # Q = V + A<-(A - Amax)
                self.Qout = self.Value + tf.subtract(self.Advantage,
                                                tf.reduce_max(self.Advantage, reduction_indices=1,
                                                              keep_dims=True))
...
class experience_buffer():
    ...
    def add(self, experience, type=0):
    ...
    def sample(self, size, reward_list = None):
        ...
        result = np.array(random.sample(self.buffer, size)), size
        return result
