...
class TrendPredictNetwork:
    ...
    def __init__(self, model_type, is_train = True, data_type = 0):
        ...
        with tf.device(device):
            with tf.variable_scope('predict_%s_%s' % (model_type, data_type)):
                ...
                self.inImage = tf.placeholder(tf.float32, shape=[None] + TrendPredictNetwork.RANGE_SHAPE[model_type][self.data_type])
    ...
    def get_image(self, code, idx, days=20):
        ...
        try:
            with pilimg.open(filepath) as im_file:
                ...
                # 가로픽셀 추가
                xPixel = self.RANGE_SHAPE[days][self.data_type][1]
                yPixel = self.RANGE_SHAPE[days][self.data_type][0]
