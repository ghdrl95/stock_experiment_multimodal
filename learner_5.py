...
class PolicyLearner:
    replay_memory = 20
    def __init__(self,load_model=True, learning_rate = 0.01, min_trading_unit=0, max_trading_unit=100, delayed_reward_threshold=.01, training = True):
        ...
        self.path = './dqn_basic_sto_renew_Q'
        ...
        self.buffer_size = ((20*(1024**3)) // (self.buffer_size*2))//10 * 10 #10GB / Imagesize
    ...
    def train(self):
        ...
        with tf.Session() as sess:
            ...
            for ii in range(self.num_episodes):
                ...
                #if delayed_reward != 0:
                accumulate = 0
                if delayed_reward == 0 and self.environment.chart_y_cnt>10:
                    delayed_reward = (- 0.1) * self.environment.chart_y_cnt
                ...
                myBuffer.add(episode_buffer.buffer,type=1)
                ...
                print("%d %d %s %s %d %d %d %d %d" % (ii, self.environment.chart_y_cnt, self.environment.chart_code, delayed_reward, self.agent.portfolio_value, self.agent.minimum_portfolio_value, self.agent.maximum_portfolio_value, self.environment.idx-self.max_epLength, self.environment.idx))
                ...
                #portfolio_list= []
                if total_steps > self.pre_train_steps and ii % 1000 == 0:
                    try:
        ...
        with tf.Session() as sess:
            ...
            for ii, code in enumerate(self.environment.chartcode_list):
                ...
                while j < self.max_epLength and not d:
                    ...
                    #입력값으로 행동선택하기(베이시안 + 볼트만)
                    #all_Q_d = np.zeros([self.agent.NUM_ACTIONS])
                    feed_dict = {self.mainQN.portfolio_state:[s_potfol], self.mainQN.temp:e, self.mainQN.keep_per:(1-e)+0.1, self.mainQN.phase:False}
                    for i, _ in enumerate(self.data_type):
                        feed_dict[self.mainQN.inImage[i]] = [s[i]]
                    Q_d = sess.run(self.mainQN.Q_dist, feed_dict=feed_dict)
                    '''
                    ...
                    total_steps += 1
                    episode_step += 1
                print("%d %d %s %s %d %d %d" % (ii, self.environment.chart_y_cnt, self.environment.chart_code, delayed_reward, self.agent.portfolio_value, self.agent.minimum_portfolio_value, self.agent.maximum_portfolio_value))
